...
namespace System
{
    ...
    public static partial class ByteExt
    {
        ...
        [Pure, PublicAPI]
        public static bool Between(this byte value, byte start, byte end, bool inclusive)
        {
            if (inclusive)
        ...
        /// <returns><see langword="true"/> if the value is in the specified range. Otherwise, <see langword="false"/>.</returns>
        [Pure, PublicAPI]
        public static bool Between(this byte value, byte start, byte end) => Between(value, start, end, true);
    }
}
